...
function SUser(conn, netkey) {
					...
					if (!info[0]) { //需要注册
						...
						js_sql.query(sql, this,
							function(info) {
								...
								if (g_config.regcard)
									this.toC("onError", {
										"msg": "欢迎来到" + g_config.appname + ",新用户奖励" + g_config.regcard + "张房卡,祝您游戏愉快!"
									});
							},
							function(err) {
								this.toC("onError", {
	...
	this.c_CreateDesk = function(agv) {
		...
		js_sql.query(sql, this, function() {
			...
			js_sql.query(sql, this, function(info) {
				...
				this.toC("CreateDeskOk", {
					"deskid": info[0]["deskid"]
				});
			}, function() {
				this.toC("onError", {
					"msg": "未找到已经创建的房间"
	...
	this.c_JiaoDZ = function(agv) {
		...
		this.m_user.onJiaoDZ(agv.basefen);
	};
	this.loadDeskDBOk = function(info) {
		...
		this.createOk = function(sub) {
				delete this.dbak;
				delete this.databak;
				this.toC("SendUserData", {
					"money": this.m_Data["money"],
					"card": this.m_Data["card"]
				});
	...
	this.c_sign = function(agv) {
			...
			this.createOk = function() {
					...
					//签到数据刷新
					this.toC("RefSignData", {
						"signdate": this.m_Data.signdate,
						"signcount": this.m_Data.signcount
					});
				}
				//发放房卡逻辑
			var signCount = this.m_Data.signcount; //累计签到次数
	...
	this.c_share = function(agv) {
		...
		this.createOk = function() {
				...
				this.toC("onError", {
					"msg": "分享成功,获得" + g_config.sharecard + "张房卡."
				});
			}
			//发放房卡逻辑
		var signDate = this.m_Data.signdate; //上一次签到时间
